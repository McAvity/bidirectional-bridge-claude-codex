...
| `.bridge-project/entry.mjs` | committed | setup (wave14) | Small entry point: reads the pin, loads the pinned runtime's launch gate in its own process. No workflow, no setup logic, no policy. `--status` (or `--instructions`) makes it report the pin, this worktree's state and the pinned runtime's instruction paths as JSON and exit; that mode resolves the pin itself, needs no `.bridge-runtime/current`, and writes nothing in any state. It requires a runtime that ships the mode: an older one answers `RUNTIME_WITHOUT_STATUS` rather than starting a server |
...
| `AGENTS.md`, managed block | may be committed | the dispatcher `setup` **only** with `--with-preference` | The optional project collaboration preference. Never written without that flag, and refused as `PREFERENCE_REQUIRES_DISPATCHER` for the legacy profile, whose worktrees have no `.bridge-project/entry.mjs` for it to name. Content outside the markers is preserved, a hand-edited block is `PREFERENCE_MODIFIED` and duplicated markers are `PREFERENCE_CONFLICT`. Its hash is recorded as `managed.preference_block`; only that exact block is later recognised, and recognising it is an observation, not consent |
