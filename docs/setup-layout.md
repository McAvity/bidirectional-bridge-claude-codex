...
| `AGENTS.md`, managed block | may be committed | `init`/`update`/`setup` **only** with `--with-preference` | The optional project collaboration preference. Never written without that flag; content outside the markers is preserved, a hand-edited block is `PREFERENCE_MODIFIED` and duplicated markers are `PREFERENCE_CONFLICT`. Its hash is recorded as `managed.preference_block` |
