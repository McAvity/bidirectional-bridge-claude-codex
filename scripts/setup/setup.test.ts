...
describe("bridge setup CLI", () => {
  ...
  // W15-I1: this CLI prepares the legacy profile, which does not install `.bridge-project/`.
  // The preference block names that entry point, so asking for it here is refused by name rather
  // than writing an instruction that points at a file this worktree will never have.
  describe("the optional project preference", () => {
    const AGENTS = "AGENTS.md";

    it("is refused for the legacy profile, before anything is written", () => {
      const project = makeProject("legacy preference project");
      init(project);
      // The named entry point genuinely does not exist here; that is the whole reason for I1.
      expect(existsSync(join(project, ".bridge-project/entry.mjs"))).toBe(false);

      const before = snapshot(project);
      const refused = bridgeJson(["init", "--workspace", project, "--runtime", runtimeA.id, "--with-preference", "--yes"]);
      expect(refused.status).toBe(1);
      expect(refused.json).toMatchObject({ ok: false, applied: false });
      const codes = (refused.json.refusals as { code: string; nextStep: string }[]).map((r) => r.code);
      expect(codes).toContain("PREFERENCE_REQUIRES_DISPATCHER");
      expect(JSON.stringify(refused.json.refusals)).toMatch(/dispatcher/u);
      // Nothing at all changed: not AGENTS.md, not the managed files, not the record.
      expect(snapshot(project)).toEqual(before);
      expect(readFileSync(join(project, AGENTS), "utf8")).toBe("# Project rules\n");
    });

    it("is never written by an ordinary init, update or rollback", () => {
      const project = makeProject("no preference project");
      const original = readFileSync(join(project, AGENTS), "utf8");
      init(project);
      ...
      expect(readFileSync(join(project, AGENTS), "utf8")).toBe(original);
    });
  });
