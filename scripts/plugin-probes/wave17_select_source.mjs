#!/usr/bin/env node
// W17-01 prototype of the feature-workflow instruction-source reader. Research instrument: the
// contract (docs/features/F-W17-workflow-plugin/contracts/01-distribution.md) fixes its
// behaviour; W17-02 moves it into the generated workflow packages.
//
//   node wave17_select_source.mjs [--cwd DIR] [--package-root DIR] [--standalone] [--json]
//
...
function parse(argv) {
  const options = { cwd: process.cwd(), packageRoot: resolve(dirname(fileURLToPath(import.meta.url)), ".."), standalone: false };
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const result = await selectSource(parse(process.argv.slice(2)));
    process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
    process.exitCode = result.source === "none" ? 3 : 0;
  } catch (error) {
