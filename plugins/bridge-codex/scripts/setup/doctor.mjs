...
function readProjectDeclaration(root) {
  try {
    return JSON.parse(readBounded(root, join(root, ".bridge-project", "bridge.json"), { maxBytes: 65536, from: "start" }).data.toString("utf8"));
  } catch {
    return null;
  }
}
...
function integrationSourceOf(root) {
  if (existsSync(join(root, PROJECT_ENTRY))) return "project-dispatcher";
  let toml = null;
  try { toml = readBounded(root, join(root, CODEX_CONFIG), { maxBytes: 1024 * 1024, from: "start" }).data; } catch { /* absent or unsafe: unknown */ }
