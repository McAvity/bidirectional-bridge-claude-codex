...
/** Distribution data is additive and untrusted, like every other package input. */
export function projectDistribution(value, aliases) {
  if (!value || value.format !== "claude-codex-bridge.distribution/v1") return null;
  const field = (v, type) => {
    // Versions and IDs are user-controlled too; known credential shapes never pass through.
    if (typeof v === "string" && SECRET_PATTERNS.some(({ re }) => new RegExp(re.source, re.flags).test(v))) return INVALID;
    return projectField(v, type, aliases);
  };
  const pick = (v, spec) => Object.fromEntries(Object.entries(spec).map(([k, type]) => [k, field(v?.[k], type)]));
  const place = (v) => !v ? null : {
    location: ["installed-runtime", "worktree", "plugin-cache", "user-home", "other"].includes(v.location) ? v.location : INVALID,
    sha256: field(v.sha256, "sha256"),
    depth: Number.isSafeInteger(v.depth) && v.depth >= 0 ? v.depth : null,
  };
  return {
    format: value.format,
    integration_source: ["project-dispatcher", "project-config", "unknown"].includes(value.integration_source) ? value.integration_source : "unknown",
    package: {
      name: value.package?.name == null ? null : ["bridge-claude", "bridge-codex"].includes(value.package.name) ? value.package.name : INVALID,
      ...pick(value.package, { version: "version", source_digest: "sha256" }),
    },
    runtime: { ...pick(value.runtime, { runtime_id: "version", commit: "hex" }), location: place(value.runtime?.location) },
    instructions: { ...pick(value.instructions, { set_sha256: "sha256", file_count: "count" }), location: place(value.instructions?.location) },
    executor: {
      configured_plugin_dir: place(value.executor?.configured_plugin_dir),
      observed_plugin_dir: place(value.executor?.observed_plugin_dir),
      ...pick(value.executor, { configured_package_version: "version", observed_package_version: "version", observed_at: "iso" }),
    },
    pin: pick(value.pin, { declared: "version", applied: "version", diverged: "flag" }),
  };
}
