...
- `objective`: feature id and round number; task IDs; governing authorization (path);
  ...
  to this round, quoted with their question id; "Follow `<package>/skills/feature-execute/SKILL.md`,
...
3. Review the delivered `HEAD`, not the branch tip; describe later integration drift separately
   ...
   an earlier runtime), also run `python3 <package>/skills/feature-exchange/scripts/feature_exchange.py
