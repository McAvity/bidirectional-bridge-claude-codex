...
Paczka workflow zawiera sześć skilli w `${CLAUDE_PLUGIN_ROOT}/skills/` i ten przewodnik. Paczka konkretnego featura zawiera jego brief oraz potrzebne materiały wejściowe. Można najpierw wprowadzić workflow jako osobny commit, następnie dodać dokumenty featura i użyć nowych skilli do planowania. Nie trzeba instalować tych skilli jako osobistych skilli ChatGPT.
